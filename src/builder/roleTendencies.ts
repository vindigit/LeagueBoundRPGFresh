...
export const derivePlayerRoleTendencies = (input: BuildRoleTendencyInput): PlayerRoleTendencies => {
  ...
  return {
    ...
    passCreationWeight: clamp(score01(passCreation) + (floorGeneral ? 0.08 : 0) + (needleThreader ? 0.06 : 0), 0.05, 1),
    selfCreationWeight: clamp(score01(selfCreation), 0.05, 1),
    offBallShotWeight: clamp(score01(shooting) * 0.88 + score01(a.stamina) * 0.12 + (catchAndShoot ? 0.08 : 0), 0.05, 1),
    rimPressureWeight: clamp(score01(rim) + (quickFirstStep ? 0.07 : 0) + (rimPressure ? 0.05 : 0), 0.05, 1),
    threeVolumeWeight: clamp(score01(shooting) + threeRelative + (deepRange ? 0.08 : 0) + (catchAndShoot ? 0.05 : 0), 0.03, 1),
    ...
    reboundWeight: clamp(reboundBase * reboundPosition * (0.72 + size * 0.55) + (glass ? 0.08 : 0), 0.03, 1),
    ...
    defensiveReboundWeight: clamp((score01(effective(a.defRebounding) * 0.78 + effective(a.strength) * 0.22) * reboundPosition * (0.72 + size * 0.58)) + (glass ? 0.08 : 0), 0.03, 1),
    stealEventWeight: clamp(score01(effective(a.stealing) * 0.5 + effective(a.perimeterDefense) * 0.28 + effective(a.speed) * 0.22) + (pointOfAttack ? 0.1 : 0) + (pickpocket ? 0.09 : 0), 0.03, 1),
    blockEventWeight: clamp(score01(effective(a.blocking) * 0.55 + effective(a.interiorDefense) * 0.28 + effective(a.strength) * 0.17) * POSITION_BLOCK_MULTIPLIER[input.position] * (0.68 + size * 0.46) + (anchor ? 0.1 : 0), 0.01, 1),
    contestWeight: clamp(score01(defense) + (pointOfAttack ? 0.05 : 0) + (anchor ? 0.06 : 0), 0.05, 1),
