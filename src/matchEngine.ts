...
const getDecisionVariance = (vision: number, rng: () => number): number => {
  const spread = tuning.decisionVarianceBase + ((99 - vision) / 99) * tuning.decisionVarianceBbiqScale;
...
const getPlayerImpact = (
  ...
  const dunking = getScaledAttribute(player.attributes.dunking, leagueLevel);
  const midrange = getScaledAttribute(player.attributes.midrange, leagueLevel);
  ...
  const interiorDefense = getScaledAttribute(player.attributes.interiorDefense, leagueLevel);
  const stealing = getScaledAttribute(player.attributes.stealing, leagueLevel);
  const blocking = getScaledAttribute(player.attributes.blocking, leagueLevel);
  ...
  const strength = getScaledAttribute(player.attributes.strength, leagueLevel);
  ...
  return {
    ...
    dunking: dunking * fatigueMultiplier,
    midrange: midrange * fatigueMultiplier,
    ...
    interiorDefense: interiorDefense * fatigueMultiplier,
    stealing: stealing * fatigueMultiplier,
    blocking: blocking * fatigueMultiplier,
    offRebounding: offRebounding * fatigueMultiplier,
    defRebounding: defRebounding * fatigueMultiplier,
    speed: speed * fatigueMultiplier,
    strength: strength * fatigueMultiplier,
    stamina,
    fatigueMultiplier,
  };
};
...
const pickBallHandlerIndex = (
  ...
  const weighted = offenseTeam.roster.map((player, index) => {
    ...
    return {
      ...
      weight: impact.handle * 0.45 + impact.passing * 0.35 + impact.vision * 0.2,
    };
  });
...
const pickShotZone = (
  ...
  const visionTilt = (shooterImpact.vision - 50) * tuning.shotZoneBbiqWeight;
  ...
  const entries: Array<{ key: ShotZone; weight: number }> = [
    {
      key: "midrange",
      weight: base.midrange + visionTilt * 0.3 - Math.abs(shootingTilt - finishingTilt) * 0.25,
    },
    {
      key: "rim",
      weight: base.rim + finishingTilt + visionTilt * 0.8 - fatigueTilt * 0.2,
    },
  ];
  ...
  if (!suppressThree) {
    entries.unshift({
      key: "three",
      weight: base.three + shootingTilt + visionTilt + fatigueTilt,
    });
  }
...
const getStealProbability = (
  ...
  clamp(
    tuning.stealBase +
      (defenderImpact.stealing * 0.65 + defenderImpact.speed * 0.35 - ballHandlerImpact.handle) /
        tuning.stealDivisor,
    tuning.stealMin,
    tuning.stealMax,
  );
...
const getBlockProbability = (
  ...
  const defenderBlockValue = defenderImpact.blocking * 0.55 + defenderImpact.speed * 0.45;
  const shooterReleaseValue = shooterImpact.shortRange * (shotZone === "rim" ? 0.7 : 0.45) + shooterImpact.midrange * 0.35;
...
const getShotMakeProbability = (
  ...
  const offenseValue =
    shotZone === "three"
      ...
      : shotZone === "midrange"
        ? shooterImpact.midrange * 0.6 + shooterImpact.shortRange * 0.2 + shooterImpact.vision * 0.2
...
const getPutbackMakeProbability = (
  ...
  const defenseValue = defenderImpact.interiorDefense * 0.65 + defenderImpact.defRebounding * 0.35;
  const variance = getDecisionVariance(shooterImpact.vision, rng);
